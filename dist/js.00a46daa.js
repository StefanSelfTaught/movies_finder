...
(function () {
  ...
  fetch('https://api.themoviedb.org/3/movie/top_rated?api_key=ce2eb2231a371296cf6ff11a39206d6e&page=1').then(function (data) {
    ...
    var topRMovies = res.results.splice(0, 6);
    var output = '';
    var genresOutput = '';
    topRMovies.map(function (movie) {
      genresOutput = _genres.genres.filter(function (genre) {
        if (genre.id === movie.genre_ids[0] || genre.id === movie.genre_ids[1]) {
          return true;
        }
      });
      output += "\n\t\t\t\t\t<div class=\"mr-3 card\" style=\"width: 14rem;\">\n\t\t\t\t        <img class=\"card-img-top\" src=\"https://image.tmdb.org/t/p/w300".concat(movie.poster_path, "\" alt=\"Card image cap\">\n\t\t\t\t        <div class=\"card-body\">\n\t\t\t\t          <h5 class=\"card-title card-movie-title\">").concat(movie.title, "</h5>\n\t\t\t\t          <p class=\"card-text\">").concat(movie.release_date.split("-")[0], " | ").concat(genresOutput[0].name, ", ").concat(genresOutput[1].name, "</p>\n\t\t\t\t        </div>\n      \t\t\t\t</div>\n\t\t\t\t\t");
    });
  ...
  fetch('https://api.themoviedb.org/3/movie/popular?api_key=ce2eb2231a371296cf6ff11a39206d6e&page=1').then(function (data) {
    ...
    var topPopularMovies = res.results.splice(0, 8);
    var output = '';
    var genresOutput = '';
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "3040" + '/');
